...
var VERSION = '1.0';
var USERS_HEAD = ['id', 'name', 'money', 'wins', 'losses', 'bestPts', 'week', 'cleared', 'updatedAt', 'updated', 'created', 'data'];
...
function save_(p) {
  ...
  if (row) {
    ...
    sh.getRange(row, 1, 1, USERS_HEAD.length).setValues([[key, keepName, data.money, data.wins, data.losses, data.bestPts, data.week, data.cleared ? 'Y' : '', at, stamp_(at), cur[10], json]]);
    return { ok: true, updatedAt: at };
  }
  sh.appendRow([key, name, data.money, data.wins, data.losses, data.bestPts, data.week, data.cleared ? 'Y' : '', at, stamp_(at), stamp_(Date.now()), json]);
