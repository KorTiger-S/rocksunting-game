...
function setupKick(i,ko){
  ...
  K.celeb=-1;K.fw=[];K.conf=[];K.fwT=0;K.emerg=false;K.shake=0;K.biteDone=false;K.hlT=0;
}
function proj(x,y,z){
  const dx=x-K.cam.x,dz=z-K.cam.z,dy=y-K.cam.y,r=dx*K.right.x+dz*K.right.z;
  const horiz=dx*K.dir.x+dz*K.dir.z,p=K.camPitch||0;
  /* 기본(p=0)일 때는 원래 수평 카메라 공식과 동일해요. 하이라이트 리플레이의 대각선 공중뷰처럼
     카메라가 아래로 내려다볼 때만 p를 줘서, 원근이 그 기울기만큼 반영되게 해요. */
  const f=p?horiz*Math.cos(p)-dy*Math.sin(p):horiz,u=p?horiz*Math.sin(p)+dy*Math.cos(p):dy;
  return{sx:400+r/f*F,sy:HOR-u/f*F,f};
}
...
/* 운명의 페널티킥 하이라이트: 총 3초. 1.5초까지는 판정 순간까지의 전체 궤적을 대각선 공중뷰로,
   나머지 1.5초는 판정 순간 전후 0.75초 구간을 골대 안 리버스 앵글로 확대 리플레이해요. */
const PK_HL_DUR=3.0,PK_HL_CUT=1.5;
function pkTiming(){
  if(K.hlT<PK_HL_CUT)return{seg:0,pt:clamp(K.hlT/PK_HL_CUT*K.tEvt,0,K.dur)};
  const p0=Math.max(0,K.tEvt-.25),p1=Math.min(K.dur,K.tEvt+.5),q=clamp((K.hlT-PK_HL_CUT)/(PK_HL_DUR-PK_HL_CUT),0,1);
  return{seg:1,pt:clamp(p0+(p1-p0)*q,0,K.dur)};
}
function startPkHighlight(){K.ph='pkhl';K.hlT=0;}
/* 하이라이트 카메라 1: 축구 중계처럼 대각선 위에서 내려다보며 공을 따라 회전하는 공중뷰 */
function pkCamA(b){
  const cam={x:9,y:6,z:-9},dx=b.x-cam.x,dz=b.z-cam.z,L=Math.hypot(dx,dz)||1,dir={x:dx/L,z:dz/L};
  return{cam,dir,right:{x:dir.z,z:-dir.x},pitch:.34};
}
/* 하이라이트 카메라 2: 골대 훨씬 뒤쪽 관중석 쯤에서 필드를 마주 보는 리버스 앵글(너무 가까우면 골키퍼가 화면을 꽉 채워버려요) */
function pkCamB(b){
  const cam={x:0,y:3.4,z:12},dx=b.x-cam.x,dz=b.z-cam.z,L=Math.hypot(dx,dz)||1,dir={x:dx/L,z:dz/L};
  return{cam,dir,right:{x:dir.z,z:-dir.x},pitch:.27};
}
...
/* 컨디션 매우나쁨(0)일 때 슛하려다 자빠질 확률과, 그때 친구들이 놀리는 대사.
   신규 유저가 초반에 너무 자주 부상 몰수패를 당한다는 피드백으로 .3→.12로 낮췄어요(킥 5번 기준 자빠질 확률 83%→47%) */
const TRIP_CHANCE=.12;
const TRIP_REACT={
...
function rollCondition(){
  const m=S.mood==null?50:S.mood,bias=m<20?-1:m>=75?1:0;
...
function updateKick(dt){
  ...
  if(K.ph==='aim'){
    ...
    if(K.pen){
      /* 운명의 페널티킥은 판정 순간 직전에 2개 카메라를 오가는 3초 하이라이트 리플레이로 넘어가요 */
      K.pt+=dt*.55;
      if(K.pt>=K.tEvt-.05){startPkHighlight();return;}
    }else{
    ...
    K.hlT+=dt;
    const pk=pkTiming();
    if(pk.seg===1&&!K.evtDone&&pk.pt>=K.tEvt){K.evtDone=true;onEvent();}
    if(K.hlT>=PK_HL_DUR){showResult();K.ph='res';K.t=0;}
  }else if(K.ph==='res'){
...
function flightT(){return K.ph==='res'?K.dur:K.ph==='pkhl'?pkTiming().pt:K.pt;}
/* 선방/미스 하이라이트가 잡히면 킥커 시점 카메라에서 골대 옆 중계 카메라로 짧게 스윕 전환해요 */
function updateCam(){
  K.camPitch=0;
  if(K.ph==='pkhl'){
    const pk=pkTiming(),b=ballAt(pk.pt),cfg=pk.seg===0?pkCamA(b):pkCamB(b);
    K.cam=cfg.cam;K.dir=cfg.dir;K.right=cfg.right;K.camPitch=cfg.pitch||0;return;
  }
...
function drawField(c){
  ...
  const kt=(K.ph==='fly'||K.ph==='pkhl')?flightT():0,tc=K.tEvt||1,kx=(K.ph==='fly'||K.ph==='pkhl')?keeperPos(flightT()):0;
  {const kp=proj(kx,0,-.4);items.push({f:kp.f,fn:()=>{
    const s=1.85*F/kp.f/69;let lift=0,rot=0;
    if(K.ph==='fly'||K.ph==='res'||K.ph==='pkhl'){const kd=K.kd,tt=flightT(),q=clamp((tt-kd.rk)/Math.max(.2,tc-kd.rk),0,1);
    ...
    const kx2=(K.ph==='res'||K.ph==='pkhl')?keeperPos(flightT()):kx,kp2=proj(kx2,0,-.4);
    c.save();c.translate(kp2.sx,kp2.sy-34*s-lift);c.rotate(rot);
    kid(c,0,34*s,s,{coat:'#e8c13a',vest:'#e8c13a',hair:'#2b1d14',arms:(K.ph==='fly'||K.ph==='pkhl')?'up':undefined});c.restore();
  ...
  const bp=(K.ph==='fly'||K.ph==='res'||K.ph==='pkhl')?ballAt(flightT()):K.ball;
  ...
  items.push({f:bq.f-.01,fn:()=>{
    ...
    if(K.ph==='fly'||K.ph==='pkhl'){const curT=flightT();for(let k=1;k<7;k++){const q=ballAt(Math.max(0,curT-k*.03)),pq=proj(q.x,q.y,q.z);c.fillStyle='rgba(255,255,255,'+(0.25-k*.03)+')';c.beginPath();c.arc(pq.sx,pq.sy,Math.max(2,.11*F/pq.f),0,7);c.fill();}}
...
function drawKick(c){
  ...
  if((K.hl&&(K.ph==='res'||(K.ph==='fly'&&K.pt>=K.tEvt-.05)))||K.ph==='pkhl'){c.fillStyle='rgba(226,51,77,.92)';rr(c,W-108,86,92,26,6);c.fill();TX(c,'REPLAY',W-62,99,15,'#fff','center');}
  if(K.ph==='aim'||K.ph==='spin'||K.ph==='power'){
  ...
  const isGoal=K.result&&K.result.type==='goal',celebOn=K.ph!=='pkhl'&&K.celeb>=0&&K.celeb<3.2,biteOn=K.ph==='res'&&!isGoal;
