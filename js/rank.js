...
async function renderRank(){
  ...
  $('#rkTab').innerHTML=list.length?list.map((x,i)=>`<tr class="${String(x.id).toLowerCase()===me?'me':''}"><td>${i+1}위 ${String(x.id).replace(/[<>&]/g,'')}${x.cleared?' 👑':''}</td><td>${RVAL[rankMetric](x)}</td></tr>`).join(''):'<tr><td>아직 기록이 없어요.</td></tr>';
