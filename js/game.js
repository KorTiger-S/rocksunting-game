...
/* ---------- 클라우드 (Supabase) ---------- */
const CLOUD_DEFAULT={url:'',key:''};   /* Supabase Project URL과 anon(public) key를 넣으면 모든 플레이어가 자동으로 연결돼요 */
function cloudCfg(){
...
async function cloudPush(){
  ...
  try{
    ...
    const r=await api('save',{id:USER.id,pin:USER.pin,updatedAt:S.updatedAt,data:cloudData()});
...
async function startLogin(raw,rawPin){
  ...
  let cloud=null;
  if(cloudUrl()){
    try{cloud=await api('load',{id,pin});}
    catch(e){
